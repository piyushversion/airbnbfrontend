...
function LoginPage(){
    ...
    async function submitHandler(event){
        ...
        const formresponse = await fetch("https://airbnbbackend1.onrender.com/login",{
        ...
        if(jsonresponse.success){
            ...
            localStorage.setItem("id",jsonresponse.data._id)
            navigate("/");
        }
        else{
                    ...
                    <div className="flex flex-col justify-between w-full text-center min-[950px]:text-justify">
                        ...
                        <form method="POST" onSubmit={submitHandler}>
                            ...
                            <div className="flex justify-center items-center gap-1.5 mb-3 min-[950px]:justify-start">
                                ...
                                <input type="text" required autoComplete="off" name="username" placeholder="Your Username" id="username" className="border-2 text-gray-500 outline-0 font-fredoka rounded px-2.5 py-1.5 bg-white w-[70%] duration-200 focus:border-2 focus:border-[#7c3aed]" onChange={changehandler}></input>
                            </div>

                            <div className="flex items-center justify-center gap-1.5 mb-3 min-[950px]:justify-start">
                                <RiLockPasswordFill size={25} color="gray"></RiLockPasswordFill>
                                <input type="password" autoComplete="off" required name="password" placeholder="Your Password" id="password" className="border-2 text-gray-500 outline-0 font-fredoka rounded px-2.5 py-1.5 bg-white w-[70%] duration-200 focus:border-2 focus:border-[#7c3aed]" onChange={changehandler}></input>
                            </div>

                            <div className="font-fredoka mb-5">Don't have an account? <span className="cursor-pointer underline" onClick={()=>navigate("/signup")}>Sign up</span></div>

                            {/* <input type="submit" value="Login" className="cursor-pointer font-fredoka text-white bg-[#3b71ca] px-8 py-2"></input> */}
